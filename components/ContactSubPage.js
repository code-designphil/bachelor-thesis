export default class ContactSubPage extends HTMLElement {
    constructor() {
        ...
        async function loadCSS() {
            const pageStyles = await fetch(
                "/components/ContactSubPage.css"
            );
    ...
    render() {
        ...
        if (this.getAttribute("subType") == "program") {
            ...
            template = document.getElementById("contact-content-and-language-page-template");
        }
        ...
        fields.forEach(field => {
            ...
            ['input', 'blur', 'submit'].forEach(eventType => {
                field.addEventListener(eventType, () => handleFormFieldInteraction(field, label))
            });
        });
